...
import chatRoom.service.ServiceFactoryInterface;
import chatRoom.service.ReceiveMessage;

public class chatRoomRMIClient {
...
//                serverPort = "1099";
                ...
                ServiceFactoryInterface sfi = (ServiceFactoryInterface) registry.lookup("ServiceFactory");
                String serviceName = sfi.getChatServiceName();
				ChatRoomServiceInterface csi = (ChatRoomServiceInterface) registry.lookup(serviceName);
			...
			try {
				...
				// 调用远程服务对象执行命令
				String[] re = csi.excuteCommand(cmd);
				for (int i = 0; i < re.length; i++) {
					System.out.println(re[i]);
				}
			} catch (NotBoundException exc) {
